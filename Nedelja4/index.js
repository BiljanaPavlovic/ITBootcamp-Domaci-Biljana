...
function addItem() {
    ...
    chkDone.value = 'done';
    chkDone.addEventListener('change', checkChange);
...
function checkChange(e) {
    ...
    if (chkBox.checked) {
        ...
        nizZadataka.push(itemContainer);
        ...
        for (let i = 0; i < nizZadataka.length; i++) {
            if (itemContainer === nizZadataka[i]) {
                delete nizZadataka[i];
            }
        }
    }
}
...
let ukloniSelektovano = document.querySelector('#remove-selected-btn');
ukloniSelektovano.addEventListener('click', removeSelected);

function removeSelected() {
    nizZadataka.forEach(element => {
        element.remove();
    });
}

let reversSelect = document.querySelector('#invert-selected-btn');
reversSelect.addEventListener('click', reversSelectF);

function reversSelectF() {
    ...
    for (let a = 0; a < inputs.length; a++) {
        ...
        inputs[a].dispatchEvent(new Event('change'));
    }
}